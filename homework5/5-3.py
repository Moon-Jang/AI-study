...
def get_normalization(_list):
    return list(map(lambda el: (el - min(_list)) / (max(_list) - min(_list)), _list))
...
location = list(zip(sepal_length_normalization_list, sepal_width_normalization_list, petal_length_normalization_list,
                    petal_width_normalization_list))

means = np.array([[0.2, 0.4, 0.1, 0.1], [0.3, 0.5, 0.5, 0.7], [0.7, 0.6, 0.4, 0.8]])
